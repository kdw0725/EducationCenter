	...
	public void employment_pro() {
		...
		try {
			...
			while(true) {
				...
				System.out.println("2. 취업 정보 등록");
				System.out.println("3. 취업 정보 수정");
				...
				if (num.equals("1")) {
					...
					sql = "{ call PROC_INSERT_EMPLOYMENT(?, ?, ?, ?, ?) }";
					
					cstat = conn.prepareCall(sql);
					
					cstat.setString(1, company);
					cstat.setString(2, salary);
					...
					sql = "{ call PROC_UPDATE_EMPLOYMENT(?, ?, ?, ?) }";
